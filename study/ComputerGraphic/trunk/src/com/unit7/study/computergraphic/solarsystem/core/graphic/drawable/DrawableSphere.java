...
public class DrawableSphere extends DrawableSpaceObject {
	...
	public void draw(GL2 gl) {
		...
		if (showOrbit) {
			...
			if (obj instanceof DrawnSphere) {
				...
				if (log.isDebugEnabled()) {
					log.debug("drawnRadius: " + sphere.getDrawnRadius()
							+ " ratio: " + camera.getRatio());
				}

				drawCircle(gl, sphere.getTarget().getX(), sphere.getTarget()
						.getY(), sphere.getDrawnRadius() * camera.getRatio(),
						80);
			}
		}
		...
		if (object instanceof DrawnSphere) {
			gl.glRotated(((DrawnSphere) object).getAngle(), 0, 1, 0);
		}
		...
		/*
		 * gl.glRasterPos3d(radius, radius, 0); GLUT glut = new GLUT();
		 * glut.glutBitmapString(GLUT.BITMAP_9_BY_15, this.getName());
		 * gl.glRasterPos3d(radius, radius - 5, 0);
		 * glut.glutBitmapString(GLUT.BITMAP_9_BY_15, "Age: " +
		 * this.getObject().getAge() + " bln. years");
		 */
	...
	protected void drawCircle(GL2 gl, double cx, double cy, double r,
			int num_segments) {
		...
		for (int ii = 0; ii < num_segments; ii++) {
			...
			if (log.isDebugEnabled()) {
				log.debug(String.format("x + cx: %.2f, y + cy: %.2f, r: %.2f",
						x + cx, y + cy, r));
			}
