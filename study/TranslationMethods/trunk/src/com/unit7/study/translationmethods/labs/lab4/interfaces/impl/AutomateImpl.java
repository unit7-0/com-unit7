...
public class AutomateImpl implements Automate {
	public AutomateImpl(State start) {
		this.start = start;
	}
	...
	public boolean checkChain(String chain) throws InformationException {
		...
		for (int i = 0; i < chain.length(); ++i) {
			...
			if (state.hasNextState(c, globalStack)) {
				...
				throw new InformationException(String.format(AutomateApp.NO_JUMP_FOR_CURRENT_STATE, c, state.getName()));
			}
		}		
		...
		log.add("\r\nchain finished. state: [ " + current.getFirst().getName() + " ] stack top: [ " + globalStack.top() + " ]");
