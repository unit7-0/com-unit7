...
public class MainForm extends JFrame {
	...
	public MainForm() {
		...
		final Box statesPanel = new Box(BoxLayout.Y_AXIS);// new JPanel(new FlowLayout());
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < comps.length; ++i) {
					if (comps[i] instanceof JTextField) {
						JTextField stateField = (JTextField) comps[i];
						int j = i + 1;
						while (j < comps.length && !(comps[i] instanceof JTextField)) {
							++j;
						}
						
						JTextField jumpField = (JTextField) comps[j];
						j = j + 1;
